...
function ISRolesList:onClick(button)
    ...
    if button.internal == "EDIT" then
        ...
        local ui = ISModalEditRole:new(50,50,400,600, self.datas.items[self.datas.selected].item);
        ui:initialise();
        ui:addToUIManager();
        return;
    end
...
function ISRolesList:onClickOption(item, action)
    if action == "DEFAULTFORBANNED" then
        setDefaultRoleFor("banned", item:getName());
        return;
    end
    if action == "DEFAULTFORNEWUSER" then
        setDefaultRoleFor("newUser", item:getName());
        return;
    end
    if action == "DEFAULTFORUSER" then
        setDefaultRoleFor("user", item:getName());
        return;
    end
    if action == "DEFAULTFORPRIORITYUSER" then
        setDefaultRoleFor("priorityUser", item:getName());
        return;
    end
    if action == "DEFAULTFOROBSERVER" then
        setDefaultRoleFor("observer", item:getName());
        return;
    end
    if action == "DEFAULTFORGM" then
        setDefaultRoleFor("gm", item:getName());
        return;
    end
    if action == "DEFAULTFOROVERSEER" then
        setDefaultRoleFor("overseer", item:getName());
        return;
    end
    if action == "DEFAULTFORMODERATOR" then
        setDefaultRoleFor("moderator", item:getName());
        return;
    end
    ...
    if action == "EDIT" then
        ...
        local ui = ISModalEditRole:new(50,50,400,600, item);
        ui:initialise();
        ui:addToUIManager();
        return;
    end
