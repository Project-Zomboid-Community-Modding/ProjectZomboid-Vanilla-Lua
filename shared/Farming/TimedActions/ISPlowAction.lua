...
function ISPlowAction:start()
    ...
    if self.item then
        ...
        if DigType.TROWEL:toString() == self.item:getDigType() then
...
function ISPlowAction:complete()
    ...
    self.character:addArmMuscleStrain(strain * strainMod)
    if bendOver then
        self.character:addBackMuscleStrain(strain * strainMod)
    end
    ...
    if (floor and floor:getProperties():has(IsoPropertyType.GRASS_FLOOR)) and sq:checkHaveGrass() == true then
	    sq:removeGrass()
	end
...
function wormCheck(character, item, square)
    ...
    -- if you use your hands or a tool that gives worms, you have a better chance of finding a worm
    local hand = not item or item:hasTag(ItemTag.DIG_WORMS)
    if hand then wormChance = wormChance / 2 end
    if ZombRand(wormChance) == 0 then
	    local worm = instanceItem(ItemKey.Food.WORM)
        if hand then
